# eherkenning-ui chart

The Helm chart installs eherkenning-ui and by default the following dependencies using subcharts:
...
```bash
helm repo add eherkenning-ui https://raw.githubusercontent.com/ConductionNL/eherkenning-ui/master/api/helm/
helm repo update
```
...
```bash
helm install my-eherkenning-ui ConductionNL/eherkenning-ui --version 0.1.0
```
...
| `settings.subdomain` | the subdomain of the installation excluding www. | `eherkenning-ui` |
| `settings.subpath` | Any sub path to follow the domain, like /api/v1 | `eherkenning-ui` |
